...
int main( int arg_count, char* arguments[] ){
	...
	// loop through the connections objects again
	// and update the sets if needed
        for (size_t i = 0; i < connections.size(); i++){
		printf("looping through\n");
        	if (fd_max < connections[i].sock){
                	fd_max = connections[i].sock;
		}
            	if (connections[i].state == eConnStateReceiving){
                	...
                	FD_SET(connections[i].sock, &write_fds);
		}else{
                	perror("Error: bad state in connection loop");
		}
        }

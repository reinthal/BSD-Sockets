...
// no blocking operations other than select() should occur. (If a blocking
...
static int setup_server_socket( short port )
{
	...
	// socket(int domain, int communication_type,int protocol)
	...
	printf( "Socket is bound to %s : %d\n", 
		...
		ntohs(actualAddr.sin_port) // network to host short
	);
